...
const PRICES={"Like a post":{adv:12,earn:4},"Like a video":{adv:12,earn:4},"Watch a video":{adv:10,earn:3},"View a video":{adv:10,earn:2.5},"Comment on a video":{adv:12,earn:5},"Custom comment":{adv:25,earn:8},"Share a post":{adv:12,earn:4},"Join a group":{adv:30,earn:7},"Follow/subscribe to a channel":{adv:30,earn:7},"Follow a page":{adv:12,earn:5},"Subscribe to a channel":{adv:30,earn:8},"Start a telegram bot":{adv:35,earn:10},"Website Signup":{adv:35,earn:10},"Website Vote":{adv:10,earn:3},"Website Visit":{adv:10,earn:3}};
...
else if(selectedApp==='TikTok'||selectedApp==='Twitter/X'){opts=["Follow a page","Like a video","View a video","Comment on a video","Custom comment","Share a post"];}
...
opts.forEach(o=>{let e=document.createElement('option');e.value=o;let p=PRICES[o]?PRICES[o].adv:30;e.textContent=o+' - ₦'+p;sel.appendChild(e);});sel.onchange=updatePrice;document.getElementById('pQty').oninput=updatePrice;updatePrice();}
...
function taskHtml(t){let app=APPS.find(a=>a.id===t.app);let img=app?`<img src="${app.logo}">`:'';let earn=t.priceEarn||20;let copyPreview='';if(t.type==='Custom comment' && t.customComments && t.customComments.length){copyPreview=`<div style="background:#fff3cd;padding:5px;margin-top:5px;border-radius:5px;border:1px dashed #0a7e07;font-size:11px">📋 ${t.customComments.length} comments - 1 per earner</div>`;}return `<div class="task-fansup"><div class="task-top"><div class="task-logo">${img}</div><div style="flex:1"><div style="font-size:12px;color:#888">${t.app} • Task</div><div style="font-weight:800;font-size:14px">${t.type}</div><div style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap"><span class="badge-gray">👥 ${t.remaining} slots left</span><span class="badge-gray">☑ Verification in 24-48h</span></div>${copyPreview}</div></div><div class="task-bottom"><div class="earn-text"><small style="color:#888;font-size:11px">Earn</small><br><b>₦${earn}</b></div><button class="start-btn" onclick="openTask('${t.id}')">Start Task</button></div></div>`;}
...
function getAssignedComment(task){
if(!task.customComments||task.customComments.length===0) return '';
let allProofs=JSON.parse(localStorage.getItem('mt_proofs')||'[]').filter(p=>p.taskId===task.id);
let index = allProofs.length % task.customComments.length;
return task.customComments[index];
}
function createTask(){
...
if(myBal<total){document.getElementById('lowBalText').innerHTML=`<b>Insufficient Balance</b><br>Current: ₦${myBal}<br>Required: ₦${total}`;document.getElementById('lowBalPopup').style.display='flex';return;}
let tasks=JSON.parse(localStorage.getItem('mt_tasks')||'[]');
let custom=[];
if(type==='Custom comment'){
let c=prompt('Enter comments separated by COMMA. If you write 10, each earner gets 1 different comment. Leave empty if not needed.');
if(c){
custom=c.split(',').map(s=>s.trim()).filter(Boolean);
}
}
...
alert('Task posted!');
document.getElementById('pLink').value='';loadMyCreatedTasks();checkLogin();showPage('post');
}
...
if(!curTask){alert('Not found');return;}
...
cl.innerHTML=`<div style="background:#fff3cd;padding:12px;border-radius:8px;border:1px dashed #0a7e07;margin-top:10px"><b>Your comment (1 of ${curTask.customComments.length}):</b><br><br><span id="copyTxt" style="font-weight:bold">${assigned}</span><br><button onclick="navigator.clipboard.writeText(document.getElementById('copyTxt').innerText);alert('Copied!')" style="margin-top:10px;background:#ff9800;color:white;border:none;padding:10px 16px;border-radius:8px;width:100%;font-weight:bold">📋 Copy This Comment</button></div>`;
}else{cl.innerHTML='';}
}
